...
static inline void Modem_SetSleep(void) {
	...
	HAL_Delay(100); // Petit délai pour laisser le temps au modem de s'endormir
}
...
ModemStatus Modem_Init_Sequence(void) {
	...
	if (Modem_Send_AT_Wait("ATE0\r", "OK", 1000) != MODEM_OK) {
	...
	if (Modem_Send_AT_Wait("AT+CMEE=2\r", "OK", 1000) != MODEM_OK) {
	...
	if (Modem_Send_AT_Wait("AT+CPIN?\r", "+CPIN: READY", 500) != MODEM_OK) {
		...
		if (Modem_Send_AT_Wait("AT+CPIN?\r", "+CPIN: SIM PIN", 500) == MODEM_OK) {
			...
			if (Modem_Send_AT_Wait(AT_PIN_CMD, "OK", 2000) != MODEM_OK) {
			...
			if (Modem_Send_AT_Wait("AT+CPIN?\r", "+CPIN: READY", 1000) != MODEM_OK) {
	...
	if (Modem_Send_AT_Wait("AT+CTZU=1\r", "OK", 1000) != MODEM_OK) {
...
ModemStatus Modem_Send_SMS(char* phone_number, char* message) {
	...
	if (Modem_Send_AT_Wait("", "OK", 10000) != MODEM_OK) {
		...
		// return ERR_SMS_BODY; On continue quand même pour faire passer en SLEEP
...
int Modem_Get_Signal_Quality(void) {
	...
	//if (Modem_Send_AT_Wait("AT+CSQ\r", "+CSQ:", 2000) == MODEM_OK) {
...
ModemStatus Modem_Init(void) {
	...
	// 2. On configure le modem pour accepter le mode veille

	Modem_WakeUp();

	// Si le modem est déjà configuré, il répondra OK quand même

	//if (Modem_Send_AT_Wait("AT+CSCLK=1", "OK", 1000) != MODEM_OK) {

	if (Modem_Send_AT_Wait("AT", "OK", 500) != MODEM_OK) {

		printf("Erreur configuration veille\n");
	...
	// 3. On autorise le modem à dormir (DTR = 1)

	// Le modem s'endormira après environ 5 secondes d'inactivité UART

	HAL_Delay(1000);

	Modem_SetSleep();

	printf("Modem en veille (en attente...)\n");
